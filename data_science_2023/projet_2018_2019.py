...
def je_fais_un_histogramme(nom_colonne):
    print("La fonction prend le nom d'une colonne et en fait l'histogramme")

def je_fais_un_donut(nom_colonne = 'MontantTotalEuros'):
    folder = Path(__file__).parent
    df = pd.read_csv(folder / '2018_2019_sac_gas.csv', sep=';')
...
def je_fais_un_diagramme_circulaire(nom_colonne):
    print("La fonction prend le nom d'une colonne et en fait le donut")
...
if __name__ == '__main__':
    folder = Path(__file__).parent
    #print(folder / "2018_2019_sac_gas.csv")
    df = pd.read_csv(folder / '2018_2019_sac_gas.csv', sep=';')
    ...
    df_montants = df["MontantTotalEuros"].value_counts()
    print(df_montants)
